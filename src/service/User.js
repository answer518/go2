...
module.exports = function(done) {
    ...
    $.method('user.add').register(async function(params, callback) {
        ...
        // 块及作用域
        {
            ...
            if(user) return callback(new Error(`User ${params.name} is already exit.`));
        }

        {
            const user = await $.method('user.get').call({email: params.email})
            if(user) return callback(new Error(`User ${params.email} is already exit.`));
        }
    ...
    $.method('user.get').register(async function(params, callback) {
        ...
        if(params._id) {
            ...
            return callback(new Error('missing parametor _id | name | email'));
        }
    ...
    $.method('user.update').register(async function(params, callback) {
        ...
        if(!user) return callback(new Error('user dones\'t exit.'));
